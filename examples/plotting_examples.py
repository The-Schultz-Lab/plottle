...
This script demonstrates how to use the plotting-helper plotting module
...
def example_heatmaps():
    ...
    # Basic heatmap
    fig, ax = heatmap(corr_matrix, title="Correlation Matrix", cmap="coolwarm", vmin=-1, vmax=1)
...
def example_publication_styling():
    ...
    configure_axes(axes[0, 0], xlabel="Value", ylabel="Frequency", title="A) Histogram")

    # Line plot
    axes[0, 1].plot(x, y1, "o-", color=colors[1], linewidth=2)
    configure_axes(axes[0, 1], xlabel="X", ylabel="Y", title="B) Line Plot")

    # Scatter
    axes[1, 0].scatter(y1, y2, color=colors[2], alpha=0.6, s=50)
    configure_axes(axes[1, 0], xlabel="Treatment A", ylabel="Treatment B", title="C) Scatter Plot")

    # Heatmap-style data
    heat_data = np.random.rand(10, 10)
    im = axes[1, 1].imshow(heat_data, cmap="viridis", aspect="auto")
    plt.colorbar(im, ax=axes[1, 1])
    configure_axes(axes[1, 1], xlabel="X", ylabel="Y", title="D) Heatmap", grid=False)
...
def example_practical_workflow():
    ...
    configure_axes(
        axes[0, 1], xlabel="Time (s)", ylabel="ln(Concentration)", title="B) Linearized Plot"
    )
    ...
    configure_axes(
        axes[1, 0], xlabel="Time (s)", ylabel="Residuals (M)", title="C) Residual Analysis"
    )

    # Panel D: Residual histogram
    axes[1, 1].hist(residuals, bins=20, color="steelblue", alpha=0.7, edgecolor="black")
    configure_axes(
        axes[1, 1], xlabel="Residual (M)", ylabel="Frequency", title="D) Residual Distribution"
    )
...
def main():
    ...
    print("plotting-helper package for scientific visualization.\n")
