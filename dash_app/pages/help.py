...
def _getting_started():
    return html.Div([
        html.Ol([
            html.Li([html.Strong("Upload Data"), " — Go to ", html.Em("Data Upload"),
                     " and load a CSV, Excel, NumPy, or other supported file. "
                     "Or click one of the 10 built-in example datasets."]),
            html.Li([html.Strong("Plot"), " — Navigate to ",
                     dbc.Button("Plot → Basic", href="/plot-basic", color="link", className="p-0",
                                style={"color": "var(--accent)"}),
                     " and select a plot type from the 26 available. Configure in the left panel."]),
            html.Li([html.Strong("Analyse"), " — ",
                     dbc.Button("Analyze → Single", href="/analyze-single", color="link", className="p-0",
                                style={"color": "var(--accent)"}),
                     " has descriptive statistics, curve fitting, signal processing, peak detection, and statistical tests."]),
            html.Li([html.Strong("Export"), " — ",
                     dbc.Button("Export", href="/export", color="link", className="p-0",
                                style={"color": "var(--accent)"}),
                     " downloads datasets as CSV/JSON/Excel, analysis results as JSON, or the full session."]),
            html.Li([html.Strong("Multi-Plot"), " — ",
                     dbc.Button("Plot → Multiplot", href="/plot-multiplot", color="link", className="p-0",
                                style={"color": "var(--accent)"}),
...
def _analysis_tools_section():
    sections = [
        ("Statistics", "Descriptive stats (mean, median, std, quartiles, IQR, range). Shapiro-Wilk normality test."),
        ("Distribution Fitting", "Fit parametric distributions (normal, exponential, gamma, lognormal, beta) and overlay PDF."),
        ("Curve Fitting", "Linear, polynomial (degree 1–10), exponential, and custom expression fits with R² and residuals."),
        ("Optimization", "Single-variable minimization (Nelder-Mead, BFGS, Powell) and root-finding over a range."),
        ("Linear Algebra", "Eigenvalue decomposition, linear system solve (Ax=b), matrix decompositions (SVD/QR/LU/Cholesky)."),
        ("Signal Processing", "Moving-average/Savitzky-Golay/Gaussian smoothing, lowpass/highpass/bandpass/bandstop filters, FFT, derivative, polynomial/rolling-ball/ALS baseline, interpolation."),
        ("Peak Analysis", "SciPy-based peak detection with height/prominence/distance thresholds, FWHM computation, peak integration, multi-peak Gaussian fitting."),
        ("Statistical Tests", "One/two-sample and paired t-tests, one-way ANOVA, Mann-Whitney U, Wilcoxon signed-rank, Kruskal-Wallis, Pearson and Spearman correlations."),
    ]
    return html.Dl([
        item
...
def _file_formats_section():
    return dbc.Table(
        [
            ...
            html.Tbody([
                html.Tr([html.Td("Tabular"), html.Td(".csv, .tsv, .xlsx, .xls, .json, .parquet"), html.Td("Auto-detected via extension")]),
                html.Tr([html.Td("NumPy"), html.Td(".npy, .npz"), html.Td("Multi-array .npz → dict of arrays")]),
                html.Tr([html.Td("Python"), html.Td(".pkl"), html.Td("Arbitrary pickled objects")]),
                html.Tr([html.Td("Spectroscopy"), html.Td(".jdx, .dx, .spc, .asc"), html.Td("JCAMP-DX, Thermo SPC, ASCII")]),
                html.Tr([html.Td("Scientific"), html.Td(".h5, .hdf5, .nc, .cdf, .mzml"), html.Td("HDF5, NetCDF, mzML mass spec")]),
            ]),
        ],
