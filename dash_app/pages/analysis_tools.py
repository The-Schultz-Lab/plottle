...
from plottle.peaks import compute_fwhm, find_peaks as pk_find_peaks, integrate_peaks
...
dash.register_page(
    __name__, path="/analyze-single", title="Analysis Tools — Plottle", name="Analysis Tools"
)
        ...
        [
            html.Div(
                [
                    ...
                    html.P(
                        "Statistical analysis, curve fitting, signal processing, and more.",
                    ...
                    dbc.Col(
                        [
                            ...
                            dbc.ButtonGroup(
                                [
                                    dbc.Button(
                                        "Descriptive Stats", id="at-stats-calc-btn", color="primary"
                                    ),
                                    dbc.Button(
                                        "Test Normality", id="at-stats-norm-btn", color="secondary"
                                    ),
                                ]
                            ),
                        ],
...
@callback(
    Output("at-stats-output", "children"),
...
def compute_stats(calc_n, norm_n, ds_name, col):
    ...
    if triggered == "at-stats-calc-btn":
        try:
            ...
            state.add_analysis_result(
                {"type": "statistics", "dataset": ds_name, "column": col, "results": stats}
            )
    ...
    if triggered == "at-stats-norm-btn":
        try:
            ...
            return dbc.Alert(
                [
                    html.Strong("Shapiro-Wilk Normality Test"),
                    html.Br(),
...
def _stats_cards(stats: dict):
    ...
    cols = [
        dbc.Col(
            html.Div(
                [
                    html.Div(f"{stats.get(k, '—'):.4g}", className="metric-value"),
...
@callback(
    Output("at-dist-output", "children"),
...
def fit_dist(n, ds_name, col, dist_name):
    ...
    try:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Histogram(x=arr, nbinsx=30, histnorm="probability density", name="Data", opacity=0.6)
        )
        ...
        fig.update_layout(
            ...
            title=f"Fitted {dist_name} distribution",
        ...
        return html.Div(
            [
                dbc.Alert(
                    [
                        html.Strong(f"Fitted {dist_name}: "),
                        params_str,
                        html.Br(),
                        f"AIC: {result.get('aic', '—'):.4f}" if "aic" in result else "",
                    ],
                    color="info",
                ),
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            dbc.Label("Poly degree (if polynomial)"),
                            dbc.Input(
                                id="at-fit-degree", type="number", value=2, min=1, max=10, size="sm"
                            ),
                        ],
                        width=3,
                    ),
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.Label("Custom expression (use x as variable)"),
                            dbc.Input(
                                id="at-fit-expr",
                                placeholder="a*np.exp(-b*x) + c",
                                type="text",
                                size="sm",
                            ),
                        ],
...
def do_fit(n, ds_name, xcol, ycol, fit_type, degree, expr):
    ...
    try:
        ...
        state.add_analysis_result(
            {"type": "curve_fit", "fit_type": fit_type, "dataset": ds_name, "results": result}
        )

        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=x, y=y, mode="markers", name="Data", marker={"size": 5, "opacity": 0.7})
        )
        if "y_fit" in result:
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            template="plotly_dark", title=f"{fit_type} Fit", paper_bgcolor="rgba(0,0,0,0)"
        )
        ...
        [
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            dbc.Label("Initial guess"),
                            dbc.Input(id="at-optim-x0", type="number", value=0, size="sm"),
                        ],
                        width=2,
                    ),
                    dbc.Col(
                        [
                            ...
                            dcc.Dropdown(
                                ...
                                options=[
                                    {"label": m, "value": m}
                                    for m in ["Nelder-Mead", "BFGS", "Powell"]
                                ],
                                value="Nelder-Mead",
                                clearable=False,
                            ),
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            dbc.Label("x range [min, max]"),
                            dbc.Row(
                                [
                                    dbc.Col(
                                        dbc.Input(
                                            id="at-roots-xmin", type="number", value=-5, size="sm"
                                        ),
                                        width=6,
                                    ),
                                    dbc.Col(
                                        dbc.Input(
                                            id="at-roots-xmax", type="number", value=5, size="sm"
                                        ),
                                        width=6,
                                    ),
                                ],
...
@callback(
    Output("at-optim-output", "children"),
...
@callback(
    Output("la-content", "children"), Input("la-tabs", "active_tab"), State("at-dataset", "value")
)
def la_tab(tab, ds_name):
    if tab == "la-eig":
        return html.Div(
            [
                ...
                dbc.Button(
                    "Compute Eigenvalues", id="la-eig-btn", color="primary", className="ms-3"
                ),
    ...
    if tab == "la-solve":
        return html.Div(
            [
                html.P(
                    "Enter matrix A and vector b as space-separated rows.",
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                dbc.Label("Matrix A (one row per line)"),
                        ...
                        dbc.Col(
                            [
                                dbc.Label("Vector b (one value per line)"),
            ...
            [
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                dbc.Label("Decomposition"),
                                dcc.Dropdown(
                                    id="la-decomp-type",
                                    options=[
                                        {"label": d, "value": d}
                                        for d in ["SVD", "QR", "LU", "Cholesky"]
                                    ],
                                    value="SVD",
                                    clearable=False,
                                ),
...
@callback(
    Output("la-solve-output", "children"),
...
@callback(
    Output("la-decomp-output", "children"),
    Input("la-decomp-btn", "n_clicks"),
    State("at-dataset", "value"),
    State("la-decomp-type", "value"),
    prevent_initial_call=True,
)
        ...
        [
                            ...
                            dcc.Dropdown(
                                ...
                                options=[
                                    ...
                                    for o in [
                                        ...
                                        "Derivative",
                                        "Baseline (Polynomial)",
                                        "Baseline (Rolling Ball)",
                                        "Baseline (ALS)",
                                        "Interpolate",
                                    ]
                                ],
                                value="Smooth (Moving Average)",
                                clearable=False,
                            ),
                        ],
                        width=5,
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            dbc.Label("Cutoff freq (0–1, for filters)"),
                            dbc.Input(
                                id="at-sig-cutoff",
                                type="number",
                                value=0.1,
    ...
    return html.Div(
        [
                ...
                [
                    ...
                    dbc.Col(
                        [
                            ...
                            dbc.Input(id="at-peaks-height", type="number", size="sm"),
                        ],
                        width=2,
                    ),
                    dbc.Col(
                        [
                            ...
                            dbc.Input(id="at-peaks-prom", type="number", size="sm"),
                        ],
                        width=2,
                    ),
                    dbc.Col(
                        [
                            ...
                            dbc.Input(id="at-peaks-dist", type="number", value=5, min=1, size="sm"),
                        ],
                        width=2,
                    ),
                ],
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Button("Find Peaks", id="at-peaks-find-btn", color="primary"),
                        width="auto",
                    ),
                    dbc.Col(
                        dbc.Button("Compute FWHM", id="at-peaks-fwhm-btn", color="secondary"),
                        width="auto",
                    ),
                    dbc.Col(
                        dbc.Button("Integrate Peaks", id="at-peaks-int-btn", color="secondary"),
                        width="auto",
                    ),
                ],
                className="g-2 mb-3",
...
@callback(
    Output("at-peaks-output", "children"),
            ...
            if len(peak_idx):
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        name="Peaks",
                        marker={"size": 8, "color": "#e0a3a3"},
                    )
                )
            fig.update_layout(
                template="plotly_dark",
                title=f"Peak Detection ({len(peak_idx)} peaks)",
                paper_bgcolor="rgba(0,0,0,0)",
            )
            return html.Div(
                [
...
@callback(
    Output("tt-content", "children"), Input("tt-tabs", "active_tab"), State("at-dataset", "value")
)
            ...
            [
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [
                                dbc.Label("Column 2 / μ₀"),
                                dbc.Input(id="tt-col2-or-mu", placeholder="col or μ₀=0", size="sm"),
                            ],
                            width=3,
                        ),
                    ],
    ...
    try:
        if triggered == "tt-ttest-btn":
            ...
            if ttype == "One-sample":
                ...
                arr2 = (
                    _get_arr(ds_name, c2_or_mu)
                    if isinstance(data, pd.DataFrame)
                    and c2_or_mu in (data.columns if isinstance(data, pd.DataFrame) else [])
                    else None
                )
                if arr2 is None:
        ...
        if triggered == "tt-anova-btn":
            groups = [
                _get_arr(ds_name, c) for c in (anova_cols or []) if _get_arr(ds_name, c) is not None
            ]
