...
from plottle.nist import fetch_ir_spectrum
...
dash.register_page(
    __name__, path="/plot-spectroscopy", title="Spectroscopy — Plottle", name="Spectroscopy"
)
        ...
        [
            html.Div(
                [
                    ...
                    html.P(
                        "IR/Raman, NMR, UV-Vis, and Mass Spectrometry analysis tools.",
        ...
        [
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.Label("Wavenumber column"),
                            dcc.Dropdown(
                                id="ir-x", options=col_opts, value=default_x, clearable=False
                            ),
                        ],
                        width=3,
                    ),
                    dbc.Col(
                        [
                            dbc.Label("Intensity column"),
                            dcc.Dropdown(
                                id="ir-y", options=col_opts, value=default_y, clearable=False
                            ),
                        ],
                        width=3,
                    ),
            ...
            html.Div(
                [
                    dbc.Row(
                        [
                            dbc.Col(
                                dbc.Button("Apply", id="ir-apply-btn", color="primary"),
                                width="auto",
                            ),
                        ],
                        ...
                        [
                            dbc.Row(
                                [
                                    dbc.Col(
                                        [
                                            dbc.Label("Crystal (ATR)"),
                                            dcc.Dropdown(
                                                id="ir-atr-crystal",
                                                options=[
                                                    {"label": c, "value": c}
                                                    for c in ["ZnSe", "Ge", "Diamond", "Si"]
                                                ],
                                                value="ZnSe",
                                                clearable=False,
                                            ),
                            ...
                            dbc.Row(
                                [
                                    dbc.Col(
                                        [
                                            dbc.Label("Background dataset"),
                                            dcc.Dropdown(id="ir-sub-bg", options=all_ds),
                                        ],
                                        width=4,
                                    ),
                                    dbc.Col(
                                        [
                                            dbc.Label("BG column"),
                                            dcc.Dropdown(id="ir-sub-bgcol", options=col_opts),
                                        ],
                                        width=3,
                                    ),
                                    dbc.Col(
                            ...
                            dbc.Row(
                                [
                                    dbc.Col(
                                        [
                                            dbc.Label("Baseline method"),
                                            dcc.Dropdown(
                                                id="ir-bl-method",
                                                options=[
                                                    {"label": m, "value": m}
                                                    for m in ["polynomial", "rubberband", "als"]
                                                ],
                                                value="polynomial",
                                                clearable=False,
                                            ),
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            dbc.Input(
                                                id="ir-nist-name", placeholder="ethanol", size="sm"
                                            ),
                                        ],
                                        width=4,
                                    ),
...
def ir_apply(
    ...
    try:
        if sub_tab == "ir-display":
            ...
            processed = atr_correction(
                x, y, n_atr=n_atr, angle_deg=float(angle or 45), n_sample=n_sample
            )
        elif sub_tab == "ir-sub":
            bg_df = _get_df(bg_ds)
            if bg_df is None:
                return dbc.Alert(
                    "Background dataset not found or not a DataFrame.", color="warning"
                )
            bg_y = (
                bg_df[bg_col].values.astype(float)
                if bg_col and bg_col in bg_df.columns
    ...
    return html.Div(
        [
            ...
            dbc.Tabs(
                ...
                id="nmr-sub-tabs",
                active_tab="nmr-display",
            ),
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.Label("Reference peak (ppm, for calibration)"),
                            dbc.Input(id="nmr-ref-ppm", type="number", value=0.0, size="sm"),
                        ],
                        width=3,
                    ),
                    dbc.Col(
                        [
                            dbc.Label("Integration region (min,max ppm)"),
                            dbc.Row(
                                [
                                    dbc.Col(
                                        dbc.Input(
                                            id="nmr-int-min", type="number", value=0.0, size="sm"
                                        ),
                                        width=6,
                                    ),
                                    dbc.Col(
                                        dbc.Input(
                                            id="nmr-int-max", type="number", value=1.0, size="sm"
                                        ),
                                        width=6,
                                    ),
                                ],
...
@callback(
    ...
    State("nmr-int-min", "value"),
    State("nmr-int-max", "value"),
    prevent_initial_call=True,
)
def nmr_apply(n, ds_name, xcol, ycol, sub_tab, ref_ppm, int_min, int_max):
    ...
    try:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=x_out, y=processed, mode="lines", name="NMR", line={"color": "#e0a3a3"})
        )
        fig.update_layout(
            template="plotly_dark",
            xaxis_title="ppm",
        ...
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.Label("Molar absorptivity ε (L/mol·cm)"),
                            dbc.Input(id="uv-eps", type="number", value=1000, size="sm"),
                        ],
                        width=3,
                    ),
                    dbc.Col(
                        [
                            dbc.Label("Path length (cm)"),
                            dbc.Input(id="uv-path", type="number", value=1.0, step=0.1, size="sm"),
                        ],
                        width=2,
                    ),
                ],
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Button("Display Spectrum", id="uv-display-btn", color="secondary"),
                        width="auto",
                    ),
                    dbc.Col(
                        dbc.Button("Beer-Lambert Concentration", id="uv-bl-btn", color="primary"),
                        width="auto",
                    ),
                    dbc.Col(
                        dbc.Button("Baseline Subtraction", id="uv-base-btn", color="secondary"),
                        width="auto",
                    ),
                ],
                className="g-2 mb-3",
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [
                            dbc.Label("Min peak height"),
                            dbc.Input(
                                id="ms-height", type="number", value=0.05, step=0.01, size="sm"
                            ),
                        ],
                        width=2,
                    ),
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Button("Display Spectrum", id="ms-display-btn", color="primary"),
                        width="auto",
                    ),
                    dbc.Col(
                        dbc.Button("Find Peaks", id="ms-peaks-btn", color="secondary"), width="auto"
                    ),
                ],
                className="g-2 mb-3",
            ),
